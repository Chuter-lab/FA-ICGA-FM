...
All numerical results reported in this paper derive from synthetic smoke-test data (randomly labeled random-texture images) and are not predictive of real FA/ICGA classification performance; the APTOS 2023 corpus requires staging on our institutional HPC cluster before real-data evaluation can proceed. FA-ICGA-FM is a pipeline for FA/ICGA classification that implements 60 methodological components spanning 15 categories, verified end-to-end on synthetic data and prepared for real-data deployment once the APTOS 2023 dataset is staged.

Standardized benchmarking infrastructure matters because re-implementation introduces heterogeneity that corrupts backbone comparisons: differences in augmentation policy, learning rate schedule, leakage handling, and calibration procedure can shift observed AUC differences between architectures by margins comparable to the architectural effect itself. Comparative evaluations in ophthalmic imaging have confirmed that apparent performance advantages between deep learning architectures can fail to generalize to independent datasets,²¹ a pattern that is undetectable when pipelines are not shared. The principal contribution of FA-ICGA-FM is a leakage-safe, modular, and open-source platform on which nine backbone architectures can be compared under identical conditions—once the APTOS 2023 corpus is staged, no group will need to re-implement backbone comparison, calibration, ordinal regression, conformal prediction, or federated learning from scratch to participate in FA/ICGA benchmarking.
