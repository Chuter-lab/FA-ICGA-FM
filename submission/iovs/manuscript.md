# FA-ICGA-FM: A Multi-Backbone, Leakage-Safe Deep Learning Pipeline for Fluorescein and ICG Angiography Classification Across 24 Retinal Conditions

---

## Title Page

**Title:** FA-ICGA-FM: A Multi-Backbone, Leakage-Safe Deep Learning Pipeline for Fluorescein and ICG Angiography Classification Across 24 Retinal Conditions
...
**Purpose:** Fluorescein angiography (FA) and indocyanine green angiography (ICGA) are foundational diagnostic modalities in retinal disease, yet automated classification across the diverse spectrum of angiographic findings remains technically demanding. We describe FA-ICGA-FM, an open-source deep learning pipeline implementing nine backbone architectures and sixty methodological components to provide reproducible, leakage-safe multi-backbone benchmarking for 24-class FA/ICGA classification on the APTOS 2023 dataset.

**Methods:** We implemented a staged pipeline encompassing (1) self-supervised pre-training via masked autoencoder (MAE) and cross-modal FA/ICGA contrastive learning; (2) supervised fine-tuning of nine backbone architectures (ViT-B/16, Swin-B, ConvNeXt-B, RETFound, BiomedCLIP, DINOv2, EfficientNetV2-S, DeiT-III-B, MaxViT-Tiny) plus CLIP zero-shot inference; (3) 11 evaluation protocols including linear probe, few-shot (5/10/25-shot), SVM-RBF probe, CORAL/CORN ordinal regression, conformal prediction, and vessel segmentation; (4) greedy ensemble selection; and (5) phase-conditioned ViT, image quality assessment, and hierarchical classification modules. All pipeline components were verified end-to-end using synthetic smoke-test data; real-data evaluation requires staging of the APTOS 2023 dataset (58,000 images).
...
Fluorescein angiography (FA) and indocyanine green angiography (ICGA) remain cornerstones of retinal diagnosis, providing complementary views of choroidal and retinal vascular anatomy that are essential for evaluating conditions ranging from age-related macular degeneration to diabetic retinopathy, central serous chorioretinopathy, and choroidal neovascularization.¹⁻² Despite their clinical importance, interpretation of FA and ICGA sequences requires substantial expertise and is subject to considerable inter-observer variation. The APTOS 2023 dataset—comprising approximately 58,000 FA and ICGA images across 24 diagnostic categories—offers an unprecedented scale for training and benchmarking automated classification systems.³

The rapid development of vision foundation models, including ViT-B/16,⁴ DINOv2,⁵ BiomedCLIP,⁶ and the ophthalmic-specific RETFound,⁷ has expanded the landscape of possible approaches to angiography classification. Foundation models have been evaluated for ophthalmic disease detection from fundus photographs and multimodal optic nerve imaging,¹⁹,²⁰ and rigorous comparative benchmarking has demonstrated that performance advantages between deep learning architectures can be substantial yet may not generalize across independent datasets.²¹ For FA/ICGA classification specifically, a unified multi-backbone benchmark with rigorous attention to leakage safety, calibration, and interpretability has not previously been described. InterpreFFA⁸ has demonstrated the value of contrastive learning for fundus fluorescein angiography interpretation, but addresses single-modality classification with a narrower architectural comparison.

We present FA-ICGA-FM, a publicly available deep learning pipeline implementing 60 methodological components across self-supervised pre-training, multi-backbone fine-tuning, a multi-protocol evaluation suite, ensemble selection, and specialized interpretability modules. Here we describe the pipeline architecture and confirm correct end-to-end execution of all 60 components, releasing FA-ICGA-FM as open-source infrastructure for reproducible, leakage-safe multi-backbone FA/ICGA benchmarking on the APTOS 2023 dataset.
...
The FA-ICGA-FM pipeline (Figure 1) is implemented in Python 3.10 with PyTorch 2.1 and operates in five sequential phases.
...
Figure 4 illustrates the distribution of 60 implemented methods across 15 methodological categories, spanning self-supervised pre-training and contrastive alignment through conformal prediction and federated learning simulation.

---

## Discussion

FA-ICGA-FM provides a validated, open-source pipeline implementing 60 methodological components across 15 categories for deep learning classification of FA and ICGA images. Standardized benchmarking infrastructure is necessary because re-implementation introduces heterogeneity that corrupts backbone comparisons: differences in augmentation policy, learning rate schedule, leakage handling, and calibration procedure can shift observed AUC differences between architectures by margins comparable to the architectural effect itself. Comparative evaluations in ophthalmic imaging have confirmed that apparent performance advantages between deep learning architectures can fail to generalize to independent datasets,²¹ a pattern undetectable when pipelines are not shared. With FA-ICGA-FM, nine backbone architectures—including general-purpose vision transformers and retinal-domain foundation models—can be compared under identical conditions; backbone comparison, calibration, ordinal regression, conformal prediction, and federated learning are all directly available without re-implementation.

All numerical values reported here derive from synthetic smoke-test data (randomly labeled random-texture images) and establish correct pipeline execution, not FA/ICGA classification performance; real-data evaluation requires staging of the APTOS 2023 corpus on our HPC cluster.
...
The APTOS 2023 dataset (58,000 images, 24 conditions) represents one of the largest publicly available FA/ICGA collections to date. Benchmarking foundation models at this scale, with the methodological breadth implemented here, has the potential to establish rigorous performance baselines for automated FA/ICGA interpretation and to identify which pre-training strategies best transfer to angiographic data. Real-data results will be reported in a follow-on paper once the dataset is staged.
...
3. Alibaba Tianchi. APTOS 2023: Fluorescence Fundus Photography Grading. https://tianchi.aliyun.com/competition/entrance/532128 (accessed July 2026).
4. Dosovitskiy A, Beyer L, Kolesnikov A, et al. An image is worth 16x16 words: Transformers for image recognition at scale. In: *Proc ICLR*. 2021.
5. Oquab M, Darcet T, Moutakanni T, et al. DINOv2: Learning robust visual features without supervision. *TMLR*. 2024.
6. Zhang S, Xu Y, Usuyama N, et al. BiomedCLIP: a multimodal biomedical foundation model pretrained from fifteen million scientific image-text pairs. *arXiv:2303.00915*. 2023.
7. Zhou Y, Chia MA, Wagner SK, et al. A foundation model for generalizable disease detection from retinal images. *Nature*. 2023;622:156-163.
8. Wang Z, Liu C, Wu X, et al. InterpreFFA: Interpretable contrastive learning for fundus fluorescein angiography analysis. *npj Digit Med*. 2025;8:PMC12222453.
9. He K, Chen X, Xie S, et al. Masked autoencoders are scalable vision learners. In: *Proc CVPR*. 2022;16000-16009.
10. Liu Z, Lin Y, Cao Y, et al. Swin transformer: Hierarchical vision transformer using shifted windows. In: *Proc ICCV*. 2021;10012-10022.
11. Liu Z, Mao H, Wu CY, Feichtenhofer C, Darrell T, Xie S. A ConvNet for the 2020s. In: *Proc CVPR*. 2022;11976-11986.
12. Tan M, Le QV. EfficientNetV2: Smaller models and faster training. In: *Proc ICML*. 2021;10096-10106.
13. Touvron H, Cord M, Douze M, Massa F, Sablayrolles A, Jégou H. Training data-efficient image transformers and distillation through attention. In: *Proc ICML*. 2021;10347-10357.
14. Tu Z, Talebi H, Zhang H, et al. MaxViT: Multi-axis vision transformer. In: *Proc ECCV*. 2022;459-479.
15. Radford A, Kim JW, Hallacy C, et al. Learning transferable visual models from natural language supervision. In: *Proc ICML*. 2021;8748-8763.
16. Cao W, Mirjalili V, Raschka S. Rank consistent ordinal regression for neural networks with application to age estimation. *Pattern Recognit Lett*. 2020;140:325-331.
17. Shi P, Cao W, Raschka S. Deep neural networks for rank-consistent ordinal regression based on conditional probabilities. *arXiv:2111.08851*. 2021.
18. McMahan B, Moore E, Ramage D, Hampson S, y Arcas BA. Communication-efficient learning of deep networks from decentralized data. In: *Proc AISTATS*. 2017;1273-1282.
19. Chuter B, Huynh J, Hallaj S, et al. Evaluating a foundation artificial intelligence model for glaucoma detection using color fundus photographs. *Ophthalmology Science*. 2025;5:100623.
20. Chuter B, Joshi V, Hallaj S, et al. A novel multimodal implementation of a foundation artificial intelligence model using optic nerve head fundus photographs and OCT imaging for glaucoma detection. *Ophthalmology Science*. 2026;6:101012.
21. Chuter B, Emmert N, Kim MY, et al. Comparison of deep learning tools for optic nerve axon quantification finds limited generalizability upon independent validation. *Bioengineering*. 2026;13:647.

---

## Figure Legends

**Figure 1.** FA-ICGA-FM pipeline architecture. The pipeline processes FA and ICGA images from the APTOS 2023 dataset (58,000 images, 24 conditions) through five sequential phases: pre-training, backbone fine-tuning, evaluation, ensemble selection, and auxiliary modules (phase-conditioned ViT, image quality assessment, hierarchical classifier). Interpretability tools (GradCAM++, attention rollout, t-SNE) and cross-modal alignment components (cross-attention FA/ICGA) are integrated across phases. All components were validated on synthetic data; real-data performance requires APTOS 2023 staging.
